...
with open(here / "handoff_config.yml", "r", encoding="utf-8") as config_file:
    handoff_config = (ruamel.yaml.safe_load(config_file) or {}).get(
        "handoff_hosts", {}
    )

handoff_url = os.getenv("HANDOFF_HOST_URL")
if handoff_url:
    handoff_key = os.getenv("HANDOFF_HOST_KEY", "configured_handoff")
    handoff_config[handoff_key] = {
        "title": os.getenv("HANDOFF_HOST_TITLE", "人工服务"),
        "url": handoff_url,
    }
    ...
    async def run(
        ...
        if not any(
            ...
            dispatcher.utter_message(
                text="我无法直接为您转接人工，但您可以通过以下渠道获取服务：",
                buttons=buttons,
            )
        return []
