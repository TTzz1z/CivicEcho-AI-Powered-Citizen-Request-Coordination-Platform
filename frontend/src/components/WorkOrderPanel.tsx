import {
  Alert, Button, Card, Descriptions, Empty, Form, Input, Modal, Select, Space, Tag, Timeline, Typography, message,
} from 'antd'
import {
...
export function WorkOrderPanel({ticket,user,onChanged}:{ticket:TicketDetail;user:User;onChanged:()=>void}){
  ...
  return <Card className="surface detail-card collaboration-panel" style={{marginTop:20}} title={<Space><ApartmentOutlined/>部门协同任务<Tag color={ticket.collaboration_status==='disputed'?'red':ticket.collaboration_status==='completed'?'green':'cyan'}>{collaborationLabels[ticket.collaboration_status]}</Tag></Space>} extra={<Space wrap>
    ...
    {!ticket.work_orders.length?<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚未创建部门处置任务"/>:<div className="work-order-grid">
      {ticket.work_orders.map(order=>{
