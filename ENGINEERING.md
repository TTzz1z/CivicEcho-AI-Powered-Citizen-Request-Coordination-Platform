...
### KB 发布事务与 Embedding 代际

- 发布路径：先 staging `_parse_and_index`，仅当 `index_status=ready` 后才 `PUBLISHED`，再撤回 `replaces_doc_id` 旧版。
- 并发：`SELECT ... FOR UPDATE`；已是 `building` 时返回 `409 INDEX_IN_PROGRESS`。
- Embedding：真实 API 可用时不把 hash fallback 向量写入 pgvector；`_vector_search` 仅取 `embedding_fallback='none'` 且 model/dimension 匹配的行。

### 工单与 WorkOrder 事务边界

`TicketService.assign/process` 与 WorkOrder 创建/启动在同一 DB session：`repository.transition(..., commit=False)`，失败则整单 `rollback`，父票状态不变。

### 对象存储与文件安全

- 生产容器内 MinIO 保持 HTTP（`OBJECT_STORAGE_SECURE=false`）；对外 HTTPS 由 Caddy 终止。
- 附件与 KB 上传共用 `get_object_storage()` / `get_kb_object_storage()`；扫描通过后才 `put`。
- ClamAV 响应严格匹配 `stream: OK` / `stream: … FOUND`。
...
- 生产 config guard：弱密钥或未启用 ClamAV 时拒绝启动；生产要求对象存储 endpoint/凭证/bucket，但**不强制** `OBJECT_STORAGE_SECURE=true`。
