...
| AI 角色辅助 | 坐席 `triage_assistant`（分诊/派发建议）与部门 `handling_assistant`（核查/文书建议）拆分；共用模型网关但 Prompt/Schema/权限不同；advisory only，不自动改状态 |
...
| AI 审计链路 | `ai_usage_logs` 按 capability 区分（含 `triage_assistant` / `handling_assistant`），每条含 `provider/model/total_tokens/latency/cost/degrade_reason/session_id` |
...
演示主闭环：坐席派发请选「综合受理」，用 `department_local` 办理。其他责任部门用于归口展示；真实环境再按部门开设账号。
