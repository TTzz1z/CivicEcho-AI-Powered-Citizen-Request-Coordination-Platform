...
test('四角色均可进入对应的 AI 辅助页面，且行政决定边界清晰',async({page})=>{
  ...
  for(const [username,path,menu,boundary] of [
    ...
    ['agent_local','/agent/intelligence','智能分诊与派发','人机协同边界'],
    ['department_local','/department/intelligence','AI 办件与文书辅助','人机协同边界'],
...
test.describe.serial('真实工单全状态闭环',()=>{
  ...
  test('坐席确认分类和优先级后受理工单',async({page})=>{
    ...
    await dialog.getByLabel('末级诉求分类').click()
    await page.locator('.ant-select-item-option').filter({hasText:/路灯故障 · CSGL-GGSS-LD/}).click()
    await dialog.getByLabel('确认优先级').click()
    await page.locator('.ant-select-item-option').filter({hasText:'紧急'}).click()
    await dialog.getByLabel('操作备注').fill('E2E 坐席已核实分类和紧急程度')
    await dialog.getByRole('button',{name:'确认提交'}).click()
    await expect(page.locator('.ant-tag').filter({hasText:'已受理'}).first()).toBeVisible({timeout:15_000})
    await expect(page.getByText('城市管理 / 公共设施 / 路灯故障')).toBeVisible()
  })

  test('坐席通过页面派发责任部门并继续协调',async({page})=>{
    await login(page,'agent_local');await expect(page).toHaveURL(/agent\/tickets/);await page.goto(`/agent/tickets/${ticketId}`)
    await page.getByRole('button',{name:'派发部门'}).click()
    const dialog=page.getByRole('dialog')
    await dialog.getByLabel('责任部门').click()
    await page.locator('.ant-select-item-option').filter({hasText:'综合受理'}).click()
    await dialog.getByLabel('操作备注').fill('E2E 派发至综合受理')
    await dialog.getByRole('button',{name:'确认提交'}).click()
    await expect(page.getByText('部门协同任务')).toBeVisible()
  ...
  test('部门人员提交工作单结果并汇总答复',async({page})=>{
    ...
    // WorkOrderPanel pre-fills result_outcome=resolved; avoid Select option clicks.
